...
bool LEVD::levd(double *in,int length) {
    bool RE = false;
    if(first)
    {
        first = false;
        last = new double[length];
        memcpy(last,in,length*sizeof(double));
        if(levd_SI!=NULL) {
            delete (levd_SI);
            levd_SI = NULL;
        }
        levd_SI = new double[length];
        if(levd_out!=NULL) {
            delete (levd_out);
            levd_out = NULL;
        }
        levd_out = new double[length];
        return RE;
    }
    else
    {
        double *temp = new double[length*2];
        memcpy(temp,last,length*sizeof(double));
        memcpy(temp+length,in,length*sizeof(double));
            Peak p = findpeaks(temp,length*2);
            double * Pks;
            int * Loc;
            bool* type;
